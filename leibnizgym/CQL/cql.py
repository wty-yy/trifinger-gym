...
class TrainConfig:
    ...
    load_model: str = "/data/user/wanqiang/document/leibnizgym/models/CQL-trifinger-e541d172/checkpoint_999999.pt"  # Model load file name, "" doesn't load
...
def train(hydra_cfg):
    ...
    state_dim=41
    action_dim=9
    dataset=np.load('/data/user/wanqiang/document/leibnizgym/dataset/mydata_v3_663750_all.npy',allow_pickle=True).item()
...
if __name__ == "__main__":
    config=TrainConfig()
    policy_file = Path(config.load_model,map_location=torch.device('cpu'))
